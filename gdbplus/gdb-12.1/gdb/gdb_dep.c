...
build_segments(void)
{
	...
	target_section_table &targetSectionVec = current_program_space->target_sections();
	for (int idx = 0; idx < targetSectionVec.size(); idx++) {
		struct target_section *p = &targetSectionVec[idx];
		...
		const char* prefix, const char* postfix)
{
	...
	else if (type->code() == TYPE_CODE_ARRAY)
	{
		...
		{
			printf_filtered(_("[%ld]"), high_bound+1);
		}
		else
			printf_filtered(_("[]\")"));
		if (field_name)
...
print_build_ids(void)
{
	...
	target_section_table &targetSectionVec = current_program_space->target_sections();
	{
		for (int i = 0; i < targetSectionVec.size(); i++) {
			struct target_section *p = &targetSectionVec[i];
			struct bfd_section *psect = p->the_bfd_section;
			bfd *pbfd = psect->owner;
