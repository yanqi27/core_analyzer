...
void register_pt_malloc_2_35() {
    ...
    if (pt::get_glibc_version(&glibc_ver_major, &glibc_ver_minor)) {
        if (glibc_ver_major == 2 && glibc_ver_minor >= 32 && glibc_ver_minor <= 41)
            my_heap = true;
    }
...
thread_tcache (struct thread_info *info, void *data)
{
	...
	tcsym = lookup_symbol("tcache", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	if (tcsym == NULL) {
...
read_malloc_state_by_symbol(address_t arena_vaddr, struct ca_malloc_state *mstate)
{
	...
	*/
	ma = lookup_symbol("main_arena", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	if (ma == NULL) {
	...
	ms_type = val->type();
	mparams.mstate_size = ms_type->length();
...
read_mp_by_symbol(void)
{
	 ...
	 */
	sym = lookup_symbol("mp_", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	if (sym == NULL) {
	...
	sym = lookup_symbol("global_max_fast", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	if (sym) {
		val = value_of_variable(sym, 0);
