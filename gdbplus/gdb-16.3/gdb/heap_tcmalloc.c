...
parse_config(void)
{
	 ...
	 */
	pageshift_ = lookup_symbol("kPageShift", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	if (pageshift_ == NULL) {
	 ...
	 */
	sizemap_ = lookup_global_symbol("tcmalloc::Static::sizemap_", nullptr,
	...
	if (!class_to_size)
		return false;
	if (ca_code(class_to_size->type()) != TYPE_CODE_ARRAY) {
		CA_PRINT("Unexpected \"class_to_size\" is not an array\n");
		return false;
	}
	if (get_array_bounds (class_to_size->type(), &low_bound,
	    &high_bound) == 0) {
    ...
    struct type *span_type)
{
	...
	if (ca_code(ptrs->type()) != TYPE_CODE_ARRAY) {
		CA_PRINT("Unexpected \"ptrs\" is not an array\n");
		return false;
	}
	if (get_array_bounds (ptrs->type(), &low_bound, &high_bound) == 0) {
	...
	for (index = low_bound; index <= high_bound; index++) {
		...
		get_array_bounds (ptrs2->type(), &low_bound2, &high_bound2);
    ...
    struct type *span_type)
{
	 ...
	 */
	pagemap = get_field_value(pageheap, "pagemap_");
	type_name = ca_name(check_typedef(pagemap->type()));
	...
	if (get_array_bounds (root->type(), &low_bound, &high_bound) == 0) {
...
parse_pagemap(void)
{
	 ...
	 */
	pageheap_ = lookup_global_symbol("tcmalloc::Static::pageheap_", nullptr,
	    SEARCH_VAR_DOMAIN).symbol;
	if (pageheap_ == NULL) {
		CA_PRINT("Failed to lookup gv "
...
parse_central_cache(void)
{
	...
	central_cache_ = lookup_global_symbol("tcmalloc::Static::central_cache_",
	    nullptr, SEARCH_VAR_DOMAIN).symbol;
	...
	if (ca_code(central_cache->type()) != TYPE_CODE_ARRAY) {
		CA_PRINT("Unexpected \"central_cache_\" is not an array\n");
		return false;
	}
	if (get_array_bounds (central_cache->type(), &low_bound,
	    &high_bound) == 0) {
	...
	for (index = low_bound; index <= high_bound; index++) {
		...
		if (tc_version_minor < 16) {
			...
			*/
			cfl_type = TYPE_BASECLASS(v->type(), 0);
...
parse_central_freelist(struct value *cfl)
{
	...
	if (ca_code(tc_slots->type()) != TYPE_CODE_ARRAY) {
		CA_PRINT("Unexpected \"tc_slots\" is not an array\n");
		return false;
	}
	if (get_array_bounds (tc_slots->type(), &low_bound,
	    &high_bound) == 0) {
...
get_field_value(struct value *val, const char *field_name)
{
	...
	fieldno = type_field_name2no(val->type(), field_name);
	if (fieldno < 0) {
...
parse_leaf(struct value *leaf, struct type *span_type)
{
	...
	if (ca_code(values->type()) != TYPE_CODE_ARRAY) {
		CA_PRINT("Unexpected: \"values\" is not an array\n");
		return false;
	}
	if (get_array_bounds (values->type(), &low_bound, &high_bound) == 0) {
...
parse_span(struct value *span)
{
	...
	if (tc_version_minor <= 6) {
		...
		int n;
		struct type *span_type = span->type();
		/*
...
parse_thread_cache(void)
{
	...
	while (value_as_address(thread_heaps_p) != 0) {
		...
		if (ca_code(lists->type()) != TYPE_CODE_ARRAY) {
			CA_PRINT("Unexpected \"list_\" is not an array\n");
			return false;
		}
		if (get_array_bounds (lists->type(), &low_bound,
		    &high_bound) == 0) {
