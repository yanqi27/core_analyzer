...
{
	g_skip_free = false;
	gdb_printf(_("Reference search will now include free heap memory blocks\n"));
}
...
{
	g_skip_free = true;
	gdb_printf(_("Reference search will now exclude free heap memory blocks (default)\n"));
}
...
include_unknown_command (const char *args, int from_tty)
{
	...
	gdb_printf(_("Reference search will now include all memory\n"));
}
...
{
	g_skip_unknown = true;
	gdb_printf(_("Reference search will now exclude memory with unknown storage type (default)\n"));
}
