...
heap_block_object_str (PyObject *obj)
{
	...
	if (format == NULL)
	{
		format = PyUnicode_FromString("heap block addr=0x%lx size=%ld inuse=%d");
		if (format == NULL)
			return NULL;
	}
...
{
	if (gdbpy_type_ready (&heap_block_object_type, gdb_module) < 0)
		return -1;
...
GDBPY_INITIALIZE_FILE(gdbpy_initialize_heap_block);
...
PyObject *gdbpy_big_blocks (PyObject *self, PyObject *args)
{
	...
	if (PyTuple_Size (args) == 1)
	{
		PyObject *obj = PyTuple_GetItem (args, 0);
		if (PyLong_Check (obj))
			n = (unsigned int) PyLong_AsLong (obj);
		else
		{
