...
object_ref_get_register_name (PyObject *self, void *closure)
{
	...
	if (obj_ref->ref.storage_type == ENUM_REGISTER)
	{
		...
		return PyUnicode_FromString (reg_name);
	}
	else
	{
		Py_INCREF (Py_None);
...
object_ref_get_module_name (PyObject *self, void *closure)
{
	...
	if (obj_ref->ref.storage_type == ENUM_MODULE_TEXT || obj_ref->ref.storage_type == ENUM_MODULE_DATA)
		return PyUnicode_FromString (obj_ref->ref.where.module.name);
	else
	{
		Py_INCREF (Py_None);
		return Py_None;
...
object_ref_str (PyObject *obj)
{
	...
	if (self->ref.storage_type == ENUM_REGISTER)
	{
		...
		format = PyUnicode_FromString("[register] thread %d %s=0x%lx");
	...
	else if (self->ref.storage_type == ENUM_STACK)
	{
		...
		format = PyUnicode_FromString("[stack] thread %d  frame %d %s");
	...
	else if (self->ref.storage_type == ENUM_MODULE_TEXT || self->ref.storage_type == ENUM_MODULE_DATA)
	{
		...
		format = PyUnicode_FromString("[.text/.data] %s %s");
	...
	else if (self->ref.storage_type == ENUM_HEAP)
	{
		if (self->ref.where.heap.inuse)
		{
			...
			format = PyUnicode_FromString("[heap block] 0x%lx--0x%lx size=%ld (type=\"%s\")");
		...
		{
			format = PyUnicode_FromString("[heap block] 0x%lx--0x%lx size=%ld free");
	...
	{
		format = PyUnicode_FromString("[unknown] 0x%lx");
		args = Py_BuildValue("(l)", self->ref.vaddr);
	}

	if (self->ref.target_index >= 0 && self->ref.value && format && args)
	{
		PyObject* target_format = PyUnicode_FromString(" 0x%lx: 0x%lx");
...
{
	if (gdbpy_type_ready (&object_ref_type, gdb_module) < 0)
		return -1;

	for (int i = 0; pyref_types[i].name; ++i)
	{
...
GDBPY_INITIALIZE_FILE(gdbpy_initialize_object_ref);
...
PyObject *gdbpy_objref (PyObject *self, PyObject *args)
{
	...
	obj = PyTuple_GetItem (args, 0);
	if (PyLong_Check (obj))
		obj_addr = (address_t) PyLong_AsLong (obj);
	else
	{
	...
	if (num_args >= 2)
	{
		...
		if (PyLong_Check (obj))
			obj_size = (address_t) PyLong_AsLong (obj);
		else
		{
	...
	if (num_args == 3)
	{
		...
		if (PyLong_Check (obj))
			stype = (enum storage_type) PyLong_AsLong (obj);
		else
		{
...
PyObject *gdbpy_global_and_static_symbols (PyObject *self, PyObject *args)
{
	...
	global_symbol_searcher spec (SEARCH_VAR_DOMAIN, ".*");
