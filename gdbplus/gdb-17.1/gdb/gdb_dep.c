...
bool get_gv_value(const char *varname, char *buf, size_t bufsz)
{
	...
	sym = lookup_static_symbol(varname, SEARCH_VAR_DOMAIN).symbol;
	if (sym) {
		struct value *val = value_of_variable(sym, 0);
		if (bufsz >= val->type()->length()) {
			gdb::array_view<const gdb_byte> content = val->contents();
			memcpy(buf, content.data(), val->type()->length());
			return true;
		}
	}
	return false;
...
static bool
thread_callback (struct thread_info *info)
{
	address_t rsp = 0;
	struct regcache *regcache = get_thread_regcache (info);
...
build_segments(void)
{
	...
	g_ptr_bit = gdbarch_ptr_bit(current_inferior()->arch());
	if (g_ptr_bit != 64 && g_ptr_bit != 32)
	{
		gdb_printf(_("Target's bit mode %d is not supported\n"), g_ptr_bit);
		return false;
	}
	...
	if (target_has_execution())
	{
		if (!linux_nat_find_memory_regions(true))
			gdb_printf(_("No memory segments found\n"));
	}
	else if (current_program_space->core_bfd() && current_program_space->core_bfd()->xvec->flavour == bfd_target_elf_flavour)
	{
		Elf_Internal_Phdr *phdr = elf_tdata(current_program_space->core_bfd())->phdr;
		g_debug_core = true;
		for (i = 0; i < elf_elfheader(current_program_space->core_bfd())->e_phnum; i++, phdr++)
		{
		...
		if (mmap_core_file(bfd_get_filename(current_program_space->core_bfd())) == false)
		{
			error (_("Failed to mmap core file %s"), bfd_get_filename(current_program_space->core_bfd()));
			return false;
		}
	}
	else
	...
	const std::vector<target_section> *targetSectionVec = default_get_section_table();
	for (int idx = 0; idx < targetSectionVec->size(); idx++) {
		const struct target_section *p = &(*targetSectionVec)[idx];
	...
	/* thread stacks */
	{
		...
		g_rsp_regnum = gdbarch_sp_regnum (current_inferior()->arch());
		/* remember current thread */
		old = inferior_ptid;
		/* switch to all threads */
		iterate_over_threads(thread_callback);
...
update_memory_segments_and_heaps(void)
{
	...
	/* set up thread context */
	{
		...
		frame_info_ptr selected_frame = get_selected_frame (NULL);
		if (selected_frame)
		{
	...
	if (g_segments && g_segment_count)
	{
		...
		if (g_debug_core || linux_nat_find_memory_regions(false))
			return true;
		gdb_printf(_("Target process has changed. Rebuild heap information\n"));
...
get_frame_number(const struct ca_segment* segment, address_t addr, int* offset)
{
	...
	address_t sp;
	frame_info_ptr fp;
	struct thread_info* tp = (struct thread_info*) segment->m_thread.context;
...
{
	address_t rsp = 0;
	struct thread_info *info = (struct thread_info*) segment->m_thread.context;
	struct regcache *regcache = get_thread_regcache (info);
...
add_addr_type(address_t addr, struct type* type, struct symbol* sym)
{
	...
	addr_type_map[addr_type_map_sz].size = type->length();
	addr_type_map_sz++;
			       ...
			       size_t bufsz, int* is_vptr)
{
	*namebuf = '\0';
	...
	if (basetype->code() == TYPE_CODE_STRUCT || basetype->code() == TYPE_CODE_UNION)
	{
		int i, num_fields, vptr_fieldno;
		struct type *vptr_basetype = NULL;
		/* sanity check of offset */
		if (offset >= basetype->length())
			return NULL;

		num_fields = basetype->num_fields();
		...
		for (i = 0; i < num_fields; i++)
		{
			...
			if (basetype->field(i).loc_kind() != FIELD_LOC_KIND_BITPOS)
				continue;
			pos = basetype->field(i).loc_bitpos() / 8;
			field_size = field_type->length();
			if ( i+1 < num_fields
				&& field_size == 1
				&& (basetype->field(i+1).loc_bitpos() / 8) == pos )
				continue;
			else if (field_size > 0 && offset >= pos && offset < pos + field_size)
			{
				...
				else if (i == vptr_fieldno && basetype == vptr_basetype)
				{
					*is_vptr = 1;
					...
					return field_type;
				else if (field_type->code() == TYPE_CODE_STRUCT || field_type->code() == TYPE_CODE_UNION)
				{
	...
	else if (basetype->code() == TYPE_CODE_ARRAY)
	{
		LONGEST low_bound, high_bound;
		if (get_array_bounds(basetype, &low_bound, &high_bound))
		{
			struct type *target_type = check_typedef (basetype->target_type());
			unsigned int elem_sz = target_type->length();
		...
		const char* prefix, const char* postfix)
{
	...
	else if (type->code() == TYPE_CODE_ARRAY)
	{
		...
		if (prefix)
			gdb_printf(_("%s"), prefix);
		print_type_name (target_type, NULL, NULL, NULL);
		LONGEST low_bound, high_bound;
		if (get_array_bounds(type, &low_bound, &high_bound))
		{
			gdb_printf(_("[%ld]"), high_bound+1);
		}
		else
			gdb_printf(_("[]\")"));
		if (field_name)
			gdb_printf(_(" %s"), field_name);
		if (postfix)
			gdb_printf(_("%s"), postfix);
		return;
	}
	...
	//if (type_name_no_tag(type))
	{
		...
		if (type->name())
			gdb_printf("struct ");
		gdb_printf(_("%s"), TYPE_SAFE_NAME(type));
		   ...
		   struct type *type, size_t offset)
{
	...
	else if (type->code() == TYPE_CODE_STRUCT || type->code() == TYPE_CODE_UNION)
	{
		...
		for (i = 0; i < num_fields; i++)
		{
			...
			else if (field_size > 0 && offset >= pos && offset < pos + field_size)
			{
				...
				if (i < num_baseclasses)
				{
					...
					gdb_printf(_(".%s"), type->field(i).name());
		...
		add_addr_type(ref->value, type->target_type(), NULL);
}
			...
			if (osect.addr() <= sect_addr &&
				...
				if (bmsymbol.minsym != NULL && bmsymbol.objfile != NULL) {
					if (sym_addr != NULL && sym_sz != NULL) {
						*sym_addr = bmsymbol.value_address();
						*sym_sz   = bmsymbol.minsym->size();
					}
			...
			*/
			...
			if (osect.addr() <= sect_addr &&
				...
				if (bmsymbol.minsym != NULL && bmsymbol.objfile != NULL) {
					...
					offset = sect_addr - bmsymbol.value_address();
					...
					sym = lookup_symbol(loc_string.c_str(), nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
					if (sym != NULL) {
	      ...
	      address_t* sym_addr, size_t* sym_sz)
{
				...
				for (struct symbol *sym : block_iterator_range (block)) {
					...
					case LOC_COMPUTED:
						try {
							...
							address_t val_addr = val->address();
	...
	return found_local_var ? rssym : NULL;
}
...
get_heap_object_type(const struct object_reference* ref)
{
	...
	if (ref->storage_type == ENUM_HEAP && ref->where.heap.inuse)
	{
		...
		/* special care is taken for heap object w/ _vptr */
			 ...
			 */
			struct symbol *sym = lookup_symbol(obj_name, nullptr, SEARCH_STRUCT_DOMAIN, nullptr).symbol;
			if (sym != NULL) {
				if (sym->type()->length() <= ref->where.heap.size &&
				    sym->type()->length() + 2 * size_t_sz >= ref->where.heap.size) {
					type = sym->type();
...
print_stack_ref(const struct object_reference* ref)
{
	...
	if (sym)
	{
		...
		gdb_printf(_(" %s"), sym->natural_name());
		print_struct_field(ref, sym->type(), ref->vaddr - sym_addr);
	}
	else
	{
		...
		if (ref->where.stack.frame >= 0)
		{
			if (ref->where.stack.offset == 0)
				...
				gdb_printf(_(" rsp%+d"), ref->where.stack.offset);
		}
		else
			gdb_printf(" below rsp");
	}

	gdb_printf(" @0x%lx", ref->vaddr);
	if (ref->value)
		gdb_printf(_(": 0x%lx"), ref->value);
}
	...
	if (ref->where.heap.inuse)
	{
		...
		if (addr_type)
		{
			...
			if (addr_type->sym)
			{
				 ...
				 */
				gdb_printf (" %s", addr_type->sym->natural_name());
			}
		...
		else if (is_heap_object_with_vptr(ref, obj_name, NAME_BUF_SZ))
		{
			...
			struct symbol *sym = lookup_symbol(obj_name, nullptr, SEARCH_STRUCT_DOMAIN, nullptr).symbol;
			if (sym != NULL &&
			    sym->type()->length() <= ref->where.heap.size &&
			    sym->type()->length() + 2 * size_t_sz >= ref->where.heap.size) {
				type = sym->type();
				print_type_name(type, NULL, " (type=\"", "\")");
				add_addr_type (ref->where.heap.addr, type, NULL);
			} else {
				gdb_printf(_(" (type=\"%s\")"), obj_name);
			}
...
{
	struct symbol *sym = lookup_symbol (var_name, nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	if (sym != NULL) {
		return sym->value_address();
	} else {
		struct bound_minimal_symbol bmsymbol;

		bmsymbol = lookup_minimal_symbol(current_program_space, var_name);
		if (bmsymbol.minsym != NULL && bmsymbol.objfile != NULL)
			return bmsymbol.value_address();
	}
	return 0;
}
...
print_build_ids(void)
{
	...
	const std::vector<target_section> *targetSectionVec = default_get_section_table();
	{
		for (int i = 0; i < targetSectionVec->size(); i++) {
			const struct target_section *p = &(*targetSectionVec)[i];
			struct bfd_section *psect = p->the_bfd_section;
			bfd *pbfd = psect->owner;
		    ...
		    size_t bufsz, size_t* type_sz)
{
	...
	if (type) {
		...
		if (type->code() == TYPE_CODE_STRUCT && type->name()) {
			char* vtbl_name;
			struct bound_minimal_symbol bmsym;

			vtbl_name = (char *)xmalloc(strlen(type->name()) + 16);
			sprintf(vtbl_name, "vtable for %s", type->name());

			bmsym = lookup_minimal_symbol (current_program_space, vtbl_name);
			if (bmsym.minsym != NULL && bmsym.objfile != NULL) {
				struct object_range* vtable = (struct object_range*)
				    malloc(sizeof(struct object_range));

				vtable->low = bmsym.value_address();
				vtable->high = vtable->low  + bmsym.minsym->size();
				vtables.push_front(vtable);
				snprintf(type_name, bufsz, "%s", type->name());
				*type_sz = type->length();
				rc = true;
			}
			xfree (vtbl_name);
...
{
	global_symbol_searcher spec(SEARCH_TYPE_DOMAIN, nullptr);
	...
	for (const symbol_search &p : symbols) {
		...
		if (p.msymbol.minsym != NULL) {
			...
			gdb_printf (_("  %s (size=%ld)\n"), tname, tsize);
		} else {
			struct type *type = p.symbol->type();
			tsize = type->length();
...
print_type_layout (char* exp)
{
    ...
    if (type)
    {
		...
		gdb_printf (_("  size=%lu\n"), type->length());
		/* if the type is pointer/reference, print the base type */
		while (type->code() == TYPE_CODE_PTR || type->code() == TYPE_CODE_REF)
		{
			type = type->target_type();
			indirect++;
		}
...
static bool
ca_get_frame_register(frame_info_ptr frame, int regnum, size_t* valp)
{
	int optimized;
	int unavailable;
	CORE_ADDR addr;
			...
			for (struct symbol *sym : block_iterator_range (b)) {
				...
				if (*linkage_name != '\0') {
					...
					bsym = lookup_symbol(linkage_name,
					    b, SEARCH_VAR_DOMAIN, nullptr);

					if (bsym.symbol->loc_class() != LOC_REGISTER)
						sym = bsym.symbol;
				}
				if (num_params)
					uiout->text(", ");
				fprintf_symbol(&stb, sym->natural_name(),
								sym->language(),
				...
				if (val)
				{
					...
					if (reg_index >= 0 && !param_regs[reg_index].has_value && !val->optimized_out())
					{
						size_t param_val = (size_t) value_as_long (val);
						param_regs[reg_index].value = param_val;
						if ((param_type->code() == TYPE_CODE_PTR || param_type->code() == TYPE_CODE_REF))
	...
	if (arg)
	{
		...
		for (i = 0; i < num_options; i++)
		{
			...
			if (*option == '%')
			{
				...
				{
					gdb_printf ("Error: unsupported register: %s\n", option);
					return;
				}
				user_regs = true;
			}
...
#define NUM_BUF_SZ 32
			...
			{
				gdb_printf ("Unknown command argument: %s\n", option);
				return;
			}
		}
	}
	...
	do
	{
		...
		if (!multi_frame || frame == frame_hi)
		{
			if (user_low)
			{
				...
				{
					gdb_printf ("Error: input start address 0x%lx is out of the function range\n", user_low);
					return;
				}
			}
			else
			...
			if (user_high)
			{
				...
				{
					gdb_printf ("Error: input end address 0x%lx is out of the function range\n", user_high);
					return;
				}
				dis_hi = user_high;
			}
		...
		if (verbose)
		{
			...
			for (i=0; i<TOTAL_REGS; i++)
			{
				...
				if (param_regs[i].has_value && i != RIP)
				{
					if (count > 0)
						gdb_printf (_(", "));
					gdb_printf (_("%s=0x%lx"), g_reg_infos[i].name, param_regs[i].value);
					if (param_regs[i].sym_name)
						gdb_printf (_("(%s)"), param_regs[i].sym_name);
					count++;
				}
			}
			if (count == 0)
		...
		gdb_printf ("End of assembler dump.\n");
						...
						|| (ptr_bit == 32 && g_reg_infos[i].preserved_x32) )
					{
						...
						if (ca_get_frame_register (selected_frame, g_reg_infos[i].gdb_regnum, &value))
						{
							if (count > 0)
								gdb_printf (_(", "));
							count++;
							gdb_printf (_(" %s=0x%lx"), g_reg_infos[i].name, value);
						}
					}
				}
				if (count == 0)
		       ...
		       address_t loc, int offset, int lea)
{
	...
	if (loc && (addr_type = lookup_type_by_addr(loc)) )
	{
		...
		if (addr_type->sym)
		{
			gdb_printf ("%s%s", lea?"&":"", addr_type->sym->natural_name());
			if (offset > 0)
			{
				if (type->code() == TYPE_CODE_PTR || type->code() == TYPE_CODE_REF)
					type = type->target_type();
	...
	if (val && val->type())
	{
		...
		if (var_len >= ptr_sz)
		{
			...
			else if (val_type == lval_register)
			{
				...
				ref.where.reg.reg_num = val->regnum();
				ref.where.reg.name = NULL;
			}
			else
				gdb_printf("Expect input expression is associated with a register or memory\n");
			...
			if (ref.vaddr)
			{
				...
				{
					gdb_printf("Failed: no in-use heap block is found\n");
				}
				else
				{
				...
				gdb_printf("Input expression doesn't reference any heap memory\n");
		}
		else
			gdb_printf("Input expression doesn't reference any heap memory(type_len=%ld)\n", var_len);
	}
	else
		gdb_printf("Can't understand the input expression\n");
}
				...
				size_t *data, bool optional)
{
	...
	*data = ULONG_MAX;
	int fieldno = type_field_name2no(val->type(), fieldname);
	if (fieldno < 0) {
		if (optional) {
			return true;
		} else {
    ...
    size_t bufsz)
{
	...
	field_val = value_field(val, fieldno);
	fieldsz = field_val->type()->length();
	if (bufsz < fieldsz) {
		CA_PRINT("Internal error: buffer of member \"%s\" is too small\n",
		    fieldname);
		return false;
	}
	if (!read_memory_wrapper(NULL, field_val->address(), buf, fieldsz)) {
