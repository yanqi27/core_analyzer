...
init_heap(void)
{
	...
	*/
	...
	// type = size_t
	sym = lookup_symbol("je_sz_large_pad", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	if (sym) {
	...
	//     unsigned int repr;
	// }
	sym = lookup_symbol("narenas_total", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	...
	if (g_version == enum_je_release::JEMALLOC_5_3) {
		sym = lookup_symbol("nbins_total", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	...
	// je_bin_infos
	// type = bin_info_t [36]
	sym = lookup_symbol("je_bin_infos", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	...
	sym = lookup_symbol("je_sz_index2size_tab", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	...
	// } [2]
	sym = lookup_symbol("rtree_levels", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	...
	//     void *repr;
	// } [4095]
	sym = lookup_symbol("je_arenas", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	if (sym == nullptr) {
	...
	for (int i = 0; i < g_jemalloc->narenas_total; i++) {
		...
		for (int j = 0; j < g_jemalloc->nbins_total; j++) {
			...
			struct type	*edata_p_type = slabcur_v->type();
	...
	if (g_version == enum_je_release::JEMALLOC_5_3) {
		sym = lookup_symbol("je_arena_emap_global", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
		if (sym == nullptr) {
		...
		sym = lookup_symbol("je_extents_rtree", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
		if (sym == nullptr) {
...
gdb_symbol_probe(void)
{
	...
	arenas = lookup_symbol("je_arenas", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	if (!arenas)
		return false;
	nbins_total = lookup_symbol("nbins_total", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	if (nbins_total)
...
static bool
thread_tcache (struct thread_info *info)
{
	struct symbol *sym;
	struct value *val;
	struct type *type;
	...
	sym = lookup_symbol("je_tsd_tls", nullptr, SEARCH_VAR_DOMAIN, nullptr).symbol;
	if (!sym) {
