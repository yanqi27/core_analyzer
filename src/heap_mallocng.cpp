...
bool parse_meta(struct value *meta, address_t address);
bool walk_sizeclass_meta(struct value *head_meta, struct type* meta_type, address_t head_address);
...
bool walk_active(struct value *malloc_context)
{
	...
	active = get_field_value(malloc_context, "active");
	...
	for (index = low_bound; index <= high_bound; index++) {
		...
		v = value_subscript(active, index);
		...
		meta = value_ind(meta_p);

		if (!walk_sizeclass_meta(meta, meta_type, value_address))
		{
			CA_PRINT("Could not parse meta at: " PRINT_FORMAT_POINTER "\n", value_address);
			return false;
		}
	}
	return true;
...
bool walk_sizeclass_meta(struct value *head_meta, struct type *meta_type, address_t head_address)
{
	...
	//head meta
	if(!parse_meta(head_meta, head_address))
	{
	...
	while(true) //gross
	{
		...
		next = value_ind(next_p);

		if(!parse_meta(next, next_address))
		{
			CA_PRINT("Could not parse sub meta\n");
			return false;
		}
		current = &g_metas[g_metas_count-1];
	}
...
bool parse_meta(struct value *meta, address_t address)
{
	...
	CA_PRINT_DBG("meta.avail_mask \"%d\"\n", my_meta->avail_mask);

	m = get_field_value(meta, "freed_mask");
	my_meta->freed_mask = value_as_long(m);
	CA_PRINT_DBG("meta.freed_mask \"%d\"\n", my_meta->freed_mask);

	m = get_field_value(meta, "last_idx");
	//m = value_ind(m);
	my_meta->slot_count = value_as_long(m);
	CA_PRINT_DBG("meta.slot_count \"%d\"\n", my_meta->slot_count);
...
int get_slot_index(struct ca_meta* meta){
}